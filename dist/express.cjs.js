...
var spdy = _interopDefault(require('spdy'));
var fs = _interopDefault(require('fs'));
var os = _interopDefault(require('os'));
var path = _interopDefault(require('path'));
var redirect = _interopDefault(require('redirect-https'));
require('colors');
...
var react$1 = (({
  ...
  const ENABLE_SW = process.env.ENABLE_SW;
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(file, 'utf8', (err, data) => {
      if (err) {
        ...
        return reject("Error loading ".concat(file)); // eslint-disable-line
      }

      const context = {}; // RENDER REACT CODE

      let content = data.replace('<!-- SSR_CONTENT -->', "<div id=\"root\">".concat(server_browser.renderToString(react.createElement(App, {
        url: req.url,
        context: context
      })), "</div>"));
      const helmet = Helmet.renderStatic(); // SET HTML ATTRS

      content = content.replace('<html>', "<html ".concat(helmet.htmlAttributes.toString(), ">")); // SET BODY ATTRS

      content = content.replace('<body>', "<body ".concat(helmet.bodyAttributes.toString(), ">")); // SET HEAD DATA

      content = content.replace('<!-- SSR_HEAD -->', "<head>\n          ".concat(helmet.meta.toString(), "\n          ").concat(helmet.title.toString(), "\n          ").concat(helmet.link.toString(), "\n          ").concat(helmet.style.toString(), "\n          ").concat(helmet.noscript.toString(), "\n          ").concat(helmet.script.toString(), "\n        </head>")); // SETUP SW

      if (ENABLE_SW) {
        console.log('Injecting Service Worker'); // eslint-disable-line

        content = content.replace('<!-- SERVICE_WORKER -->', "<script>\n            if ('serviceWorker' in navigator) {\n              navigator.serviceWorker.register('/service-worker.js').then((registration) => {\n                this.serviceWorkerRegistration = registration;\n              });\n            }\n          </script>");
      } else {
...
var vanilla = (({
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(file, 'utf8', (err, data) => {
      ...
      let content = data.replace('<!-- SSR_CONTENT -->', '<div id="root">Vanilla...</div>'); // SETUP SW
...
const server = ({
  ...
  let TEMPLATE = template; // DEFINE VARS

  const app = express();
  const watcher = chokidar.watch("".concat(ROOT, "./src"));
  const certDirectory = path.join(os.homedir(), '.nodecert');
  const DIR = {
    common: "".concat(ROOT, "/src/common/"),
    public: "".concat(ROOT, "/public"),
    dist: "".concat(ROOT, "/.dist"),
    functions: "".concat(ROOT, "/.functions"),
    src: "".concat(ROOT, "/src"),
    root: "".concat(ROOT, "/")
  };
  app.use(compression());
  app.use(cors()); // UPDATE SERVER WHEN FILES CHANGE
  ...
  if (HOST === 'firebase`') {
    TEMPLATE = "".concat(path.join(__dirname, '../'), ".dist/template.html");
    staticDirs.dist = "".concat(path.join(__dirname, '../'), ".dist");
    staticDirs.src = "".concat(path.join(__dirname, '../'), "public");
  } else if (HOST !== 'local') {
    TEMPLATE = "".concat(path.join(__dirname, '../../'), "src/template.html");
    staticDirs.dist = "".concat(path.join(__dirname, '../../'), ".dist");
    staticDirs.src = "".concat(path.join(__dirname, '../../'), "public");
  }

  app.use(express.static(staticDirs.dist));
  app.use(express.static(staticDirs.dist)); // RENDER REACT APP

  app.get('/*', (req, res) => {
    try {
      const streamCSS = res.push("".concat(DIR.dist, "/app.css"), {
        status: 200,
        // optional
        method: 'GET',
        // optional
      ...
      streamCSS.end();
      const streamJS = res.push("".concat(DIR.dist, "/app.js"), {
        status: 200,
        // optional
        method: 'GET',
        // optional
      ...
      streamJS.end();
    } catch (err) {// SERVER PUSH NOT SUPPORTED
    }
  ...
  if (keys) {
    initServer = SERVER_PORT => {
      spdy.createServer(keys, app).listen(SERVER_PORT, error => {
        ...
        const host = "https://localhost:".concat(SERVER_PORT);
        return console.log("\n Node server started at ".concat(host, "\n")); // eslint-disable-line
      });
    };
  } else {
    initServer = SERVER_PORT => {
      app.listen(SERVER_PORT, () => {
        const host = "http://localhost:".concat(SERVER_PORT);
        console.log("\n Node server started at ".concat(host, "\n"));
      });
    };
  }
